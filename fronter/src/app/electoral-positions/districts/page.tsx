...
const DistrictElections: React.FC = () => {
  ...
  const [addDistrictCandidate] = useAddDistrictCandidateMutation();
  const [updateDistrictCandidate] = useUpdateDistrictCandidateMutation();
  const [deleteDistrictCandidate] = useDeleteDistrictCandidateMutation();
  const { data: districtCandidates, refetch } = useGetDistrictCandidatesQuery(
    {}
  );

  const [electionType, setElectionType] = useState("");
  const [candidateData, setCandidateData] = useState<DistrictCandidate>({
    id: "",
    ninNumber: "",
    firstName: "",
    lastName: "",
  ...
  const councilorTypeOptions = {
    partyStructure: [],
    lcv: [],
    DistrictCouncillors: ["Direct councillors", "Female councillors"],
    DistrictSIGCouncillors: [
      "Youth councilor",
      "PWD councilor",
      "Elders councilor",
  ...
  const handleElectionTypeChange = (
    ...
    setCandidateData({
      ...candidateData,
      districtElectionType: e.target.value,
      category: "",
      position: "",
      councilorType: "",
      gender: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const missingFields = requiredFields.filter(
      (field) => !candidateData[field as keyof DistrictCandidate]
    );
    ...
    try {
      const dataToSubmit: Partial<DistrictCandidate> = {
        ...candidateData,
      };
      ...
      Object.keys(dataToSubmit).forEach((key) => {
        if (dataToSubmit[key as keyof DistrictCandidate] === undefined) {
          delete dataToSubmit[key as keyof DistrictCandidate];
        }
      });

      if (editMode) {
        await updateDistrictCandidate({
          ...dataToSubmit,
          id: candidateData.id,
        }).unwrap();
      } else {
        await addDistrictCandidate(dataToSubmit).unwrap();
      }
      refetch();
      resetForm();
  ...
  const handleEdit = (candidate: DistrictCandidate) => {
      ...
      case "DistrictCouncillors":
        return (
          ...
          >
            ...
            {councilorTypeOptions.DistrictCouncillors.map((option) => (
              <option key={option} value={option}>
                {option}
              </option>
            ))}
      ...
      case "DistrictSIGCouncillors":
        return (
          <>
            ...
            >
              <option value="">Select Councilor Type</option>
              {councilorTypeOptions.DistrictSIGCouncillors.map((option) => (
                <option key={option} value={option}>
                  {option}
                </option>
              ))}
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">
        District Elections Candidate Information
      </h1>
      ...
      >
        <div className="mb-4">
          ...
          >
            ...
            <option value="lcv">LCV Elections</option>
            <option value="DistrictCouncillors">District Councillors</option>
            <option value="DistrictSIGCouncillors">
              District SIG Councillors
            </option>
          </select>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filterCandidatesByType(activeTab)?.map(
                (candidate: DistrictCandidate, idx: number) => (
                  <tr key={idx}>
...
export default DistrictElections;
