...
const SubcountiesDivisionsElections: React.FC = () => {
  ...
  const [addSubcountiesDivisionsCandidate] =
    useAddSubcountiesDivisionsCandidateMutation();
  const [updateSubcountiesDivisionsCandidate] =
    useUpdateSubcountiesDivisionsCandidateMutation();
  const [deleteSubcountiesDivisionsCandidate] =
    useDeleteSubcountiesDivisionsCandidateMutation();
  const { data: subcountiesDivisionsCandidates, refetch } =
    useGetSubcountiesDivisionsCandidatesQuery({});

  const [electionType, setElectionType] = useState("");
  const [candidateData, setCandidateData] =
    useState<SubcountiesDivisionsCandidate>({
      id: "",
      ninNumber: "",
      firstName: "",
      lastName: "",
  ...
  const handleElectionTypeChange = (
    ...
    setCandidateData({
      ...candidateData,
      subcountiesDivisionsElectionType: e.target.value,
      category: "",
      position: "",
      councilorType: "",
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const missingFields = requiredFields.filter(
      (field) => !candidateData[field as keyof SubcountiesDivisionsCandidate]
    );
    ...
    try {
      const dataToSubmit: Partial<SubcountiesDivisionsCandidate> = {
      ...
      Object.keys(dataToSubmit).forEach((key) => {
        if (
          dataToSubmit[key as keyof SubcountiesDivisionsCandidate] === undefined
        ) {
          delete dataToSubmit[key as keyof SubcountiesDivisionsCandidate];
        }
      });
      ...
      if (editMode) {
        ...
        await addSubcountiesDivisionsCandidate(dataToSubmit).unwrap();
      }
      refetch();
      resetForm();
  ...
  const handleEdit = (candidate: SubcountiesDivisionsCandidate) => {
    setCandidateData(candidate);
    setElectionType(candidate.subcountiesDivisionsElectionType);
    setEditMode(true);
  };
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Are you sure you want to delete this candidate?")) {
      try {
        await deleteSubcountiesDivisionsCandidate(id).unwrap();
        refetch();
      ...
      case "SubcountySIGCouncillors":
        return (
          <select
            className="block w-full p-2 mt-2 border rounded-md shadow-sm focus:border-yellow-500"
            name="councilorType"
            value={candidateData.councilorType}
            onChange={handleInputChange}
          >
            <option value="">Select Councilor Type</option>
            {councilorTypeOptions[
              electionType as keyof typeof councilorTypeOptions
            ].map((option) => (
              <option key={option} value={option}>
                {option}
              </option>
  ...
  const filterCandidatesByType = (type: string) => {
    if (type === "all") return subcountiesDivisionsCandidates;
    return subcountiesDivisionsCandidates?.filter(
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">
        Subcounties/Divisions Elections Candidate Information
      </h1>
      ...
      >
        <div className="mb-4">
          ...
          >
            ...
            <option value="lc3">LC3 Elections</option>
            <option value="SubcountyCouncillors">Subcounty Councillors</option>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filterCandidatesByType(activeTab)?.map(
                (candidate: SubcountiesDivisionsCandidate, idx: number) => (
                  <tr key={idx}>
...
export default SubcountiesDivisionsElections;
