...
const VillageCellOpposition: React.FC = () => {
  const [operationResult, setOperationResult] = useState<{
    success: boolean;
    message: string;
  } | null>(null);

  const { data: villageCellCandidates } = useGetVillageCellsCandidatesQuery({});
  ...
  const [oppositionCandidate, setOppositionCandidate] =
    useState<OppositionCandidate>({
      ...
      ward: "",
      cell: "",
      villageCellElectionType: "partyStructure",
      vote: 0,
      party: "",
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editMode && oppositionCandidate.id) {
        result = await updateOppositionCandidate(dataToSubmit).unwrap();
      } else {
        result = await createOppositionCandidate(dataToSubmit).unwrap();
      }
  ...
  const getCategoryKey = (candidate: OppositionCandidate) => {
    switch (candidate.villageCellElectionType) {
      case "partyStructure":
  ...
  const groupWinnersByTypeAndCategory = (candidates: any[]) => {
    ...
    candidates.forEach((candidate) => {
      if (candidate.isQualified) {
        ...
        if (
          !grouped[candidate.villageCellElectionType][category] ||
          candidate.vote >
            grouped[candidate.villageCellElectionType][category].vote
        ) {
          grouped[candidate.villageCellElectionType][category] = candidate;
        }
      }
    });
    return grouped;
  ...
  const sortedGroupedWinners = useMemo(() => {
    return groupWinnersByTypeAndCategory(villageCellCandidates || []);
  ...
  const renderWinnerTable = (winner: any, category: string) => (
    <div
      key={`${winner.villageCellElectionType}-${category}-winner`}
      className="mb-8"
    >
  ...
  const renderOppositionTable = (
    ...
    <div
      key={`${oppositionCandidates[0].villageCellElectionType}-${category}-opposition`}
      className="mb-8"
    >
        ...
        <button
          onClick={() => {
            ...
            setOppositionCandidate({
              ...
              cell: "",
              villageCellElectionType:
                oppositionCandidates[0].villageCellElectionType,
              vote: 0,
              party: "",
            });
          }}
  ...
  const renderFormFields = () => {
    switch (oppositionCandidate.villageCellElectionType) {
      case "partyStructure":
        return (
          <>
            <select
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">
        Village/Cell Opposition Leaders
      </h1>
                      ...
                      <button
                        onClick={() => {
                          ...
                          setOppositionCandidate({
                            ...
                            ward: "",
                            cell: "",
                            villageCellElectionType: type as
                              | "partyStructure"
                              | "lc1",
                            vote: 0,
                            party: "",
        ...
        : Object.entries(sortedGroupedWinners[activeTab] || {}).map(
            ([category, winner]) => (
              <div key={`${activeTab}-${category}`}>
                ...
                oppositionCandidates.filter(
                  (candidate: OppositionCandidate) =>
                    candidate.villageCellElectionType === activeTab &&
                    getCategoryKey(candidate) === category
                ).length > 0 ? (
                  renderOppositionTable(
                    oppositionCandidates.filter(
                    ...
                    <button
                      onClick={() => {
                        ...
                        setOppositionCandidate({
                          ...
                          ward: "",
                          cell: "",
                          villageCellElectionType: activeTab as
                            | "partyStructure"
                            | "lc1",
                          vote: 0,
                          party: "",
          ...
          <div className="bg-white p-8 rounded w-full max-w-3xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <select
                  name="villageCellElectionType"
                  value={oppositionCandidate.villageCellElectionType}
                  onChange={handleInputChange}
...
export default VillageCellOpposition;
