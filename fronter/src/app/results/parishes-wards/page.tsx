...
const ParishWardResults = () => {
  ...
  const groupCandidates = (candidates: Candidate[]) => {
    const grouped: {
      [key: string]: { [key: string]: { [key: string]: Candidate[] } };
    } = {};
    candidates.forEach((candidate) => {
      if (candidate.isQualified) {
        ...
        if (!grouped[type][category][position])
          grouped[type][category][position] = [];
    ...
    Object.keys(groupedCandidates).forEach((type) => {
      Object.keys(groupedCandidates[type]).forEach((category) => {
        Object.keys(groupedCandidates[type][category]).forEach((position) => {
          groupedCandidates[type][category][position].sort(
            (a, b) => b.vote - a.vote
          );
        });
      });
  ...
  const renderElectionTypeContent = (
    type: string,
    categoryPositions: { [key: string]: { [key: string]: Candidate[] } }
  ) => {
    return (
      <div key={type}>
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {activeTab === "all"
        ...
        : renderElectionTypeContent(
            activeTab,
            sortedGroupedCandidates[activeTab] || {}
          )}
